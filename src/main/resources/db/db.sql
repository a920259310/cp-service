...
DROP TABLE IF EXISTS `cp_data_sys_config`;

CREATE TABLE `cp_data_sys_config` (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT '主键',
  `game_type` enum('1','2','3') DEFAULT '1' COMMENT '1:重 2:北 3:天',
  `plan_count` int(11) DEFAULT '0' COMMENT '计划次数',
  `recent_day` int(11) DEFAULT '0' COMMENT '统计最大连续次数时,使用最近几天的数据',
  `lg_max_count` int(11) DEFAULT '0' COMMENT '预计最大次数大于max_count多少次',
  `max_count` int(11) DEFAULT '0' COMMENT '最近N天的最大次数',
  `max_count_count` int(11) DEFAULT NULL COMMENT '最近N天最大次数出现的次数',
  `max_count_date_time_max` timestamp NULL DEFAULT NULL COMMENT '最近N天最大次数出现的最大时间',
